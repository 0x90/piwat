...
"""
import sys, optparse, re, time, random, pdb, os, PyLorcon2
...
def lorconTX(pktNum=5,packet=None, channel=1 ,slept=0):
	...
	#we don't need to keep setting it to inject mode -- that takes time.
	try:
		if inject != 1:
			tx.open_injmon()
			inject = 1
	...
	while count != pktNum:
		try:
			...
			message.printMessage(["\nLorcon error.  Are you using correct driver?"])
			#message.printMessage(['\nError Message from lorcon:',str(e),
